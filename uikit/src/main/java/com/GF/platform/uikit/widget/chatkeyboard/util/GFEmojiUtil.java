...
import com.GF.platform.uikit.widget.chatkeyboard.base.entity.GFEmojiSpan;
...
public class GFEmojiUtil {
    ...
    public static SpannableStringBuilder getFace(Context context, String png, TextView tv) {
        ...
        try {
            ...
            drawable.setBounds(0, 0, GFKeyBoardUtil.getFontHeight(tv), GFKeyBoardUtil.getFontHeight(tv));
            GFEmojiSpan span = new GFEmojiSpan(drawable);
    ...
    public static boolean isDeletePng(String content) {
        if (content.length() >= GFEmojiUtil.emojiFormat.length()) {
            String checkStr = content.substring(content.length() - GFEmojiUtil.emojiFormat.length(),
                    content.length());
            Pattern p = Pattern.compile(GFEmojiUtil.emojiRegex);
