...
public class GFCustomTextView extends TextView {
    ...
    public GFCustomTextView(Context context) {
        super(context);
    }

    public GFCustomTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        // super一定要在我们的代码之前配置文件
        init(context, attrs, 0);
    }

    public GFCustomTextView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.GFCustomTextView, defStyle, 0);
        if (a != null) {
            // 获得参数个数
            ...
            for (int i = 0; i < count; i++) {
                index = a.getIndex(i);
                if (index == R.styleable.GFCustomTextView_bottom_height) {
                    bottomHeight = a.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.GFCustomTextView_bottom_width) {
                    bottomWidth = a.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.GFCustomTextView_left_height) {
                    leftHeight = a.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.GFCustomTextView_left_width) {
                    leftWidth = a.getDimensionPixelSize(index, -1);
                }else if (index == R.styleable.GFCustomTextView_right_height) {
                    rightHeight = a.getDimensionPixelSize(index, -1);
                }else if (index == R.styleable.GFCustomTextView_right_width) {
                    rightWidth = a.getDimensionPixelSize(index, -1);
                }else if (index == R.styleable.GFCustomTextView_top_height) {
                    topHeight = a.getDimensionPixelSize(index, -1);
                }else if (index == R.styleable.GFCustomTextView_top_width) {
